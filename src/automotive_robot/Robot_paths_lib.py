...
def find_AH_paths(ox_b, oy_b, startpoint, goal):
    ...
    for start_point in AH_points:
        AH_path_temp = get_possible_AH_next_points_from_point(
            AH_points, obstacle_list, start_point, goal)
...
def BFS_skeleton_path(graph, start, goal):
    ...
    while queue:
        path = queue.pop(0)
        node = path[-1]     # lấy phần tử cuối cùng
...
def approximately_shortest_path(skeleton_path, traversal_sight, robot_vision):
    if len(skeleton_path) <= 2:
        ...
        critical_ls = get_critical_ls(
            skeleton_path, traversal_sight, robot_vision)
...
def get_active_open_points(local_open_pts, traversal_sights, robot_vision, center, goal):
    ...
    if len(local_open_pts):  # new local found
        if ENABLE_AR_RANKING:
            local_active_open_pts = is_inside_active_arc(
                local_open_pts, robot_vision, center, goal)
        else:
            local_active_open_pts = local_open_pts

        # remove local_point which is inside explored area
        if len(traversal_sights) > 0:
            local_open_pts_status = [inside_global_true_sight(
                pt, robot_vision, traversal_sights) for pt in local_active_open_pts]
            local_active_open_pts = local_active_open_pts[np.logical_not(
                local_open_pts_status)]
...
def store_global_active_points(g_active_open_pts, l_active_open_pts, ranking_score):
    if len(l_active_open_pts) > 0:
        local_active_open_pts_info = np.concatenate(
            (l_active_open_pts, ranking_score), axis=1)
        if len(g_active_open_pts) == 0:
            g_active_open_pts = np.array(local_active_open_pts_info)
        else:
            g_active_open_pts = np.concatenate(
                (g_active_open_pts, local_active_open_pts_info), axis=0)
