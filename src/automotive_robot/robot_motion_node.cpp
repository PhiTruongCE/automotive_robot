...
void move(double distance, automotive_robot::Point &next_point) {
    ...
    (cpos.x - next_point.x)*(cpos.x - next_point.x) + (cpos.y - next_point.y)*(cpos.y - next_point.y) > stable_moving_tolerance);

    vel_msg.linear.x = MAX_MV_SPEED;    // force velocity to be maximum bc velocity might not be maximum 
