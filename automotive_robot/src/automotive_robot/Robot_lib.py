...
def left_hand_direction(obstacle):
    ...
    if(vector_a[0] > 0 and vector_a[1] > 0):        #sang phai
        if (vector_b[0] > 0 and vector_b[1] < 0):
            return False
        elif (vector_b[0] < 0 and vector_b[1] > 0):
            return True
        else:
            relative_upward_vector = (- vector_a[1], vector_a[0])
            if (relative_upward_vector[0]*vector_b[0] + relative_upward_vector[1]*vector_b[1] > 0):
                return True     # nguoc chieu
            else:
                return False    # cung chieu
    ...
    if(vector_a[0] < 0 and vector_a[1] > 0):        #sang trai
        if (vector_b[0] > 0 and vector_b[1] > 0):
            return False
        elif (vector_b[0] > 0 and vector_b[1] < 0):
            relative_upward_vector = (vector_a[1], - vector_a[0])
